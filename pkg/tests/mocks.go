...
func (m *MockCli) Execute(mask bool, args ...string) (out string, err error) {
	m.Calls = append(m.Calls, args)
	length := len(m.Calls)
	return m.StoredResponse[length-1], m.StoredErrors[length-1]
}

func (m *MockCli) ExecuteAndStreamOutput(mask bool, output cli.OutputHandler, args ...string) (err error) {
	m.Calls = append(m.Calls, args)
	length := len(m.Calls)
